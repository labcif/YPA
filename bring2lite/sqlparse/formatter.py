...
from filterstokens import KeywordCaseFilter
from filterstokens import IdentifierCaseFilter
from filterstokens import TruncateStringFilter
...
def build_filter_stack(stack, options):
    ...
    Args:
      stack: :class:`~sqlparse.FilterStack` instance
      options: Dictionary with options validated by validate_options.
    """
    # Token filter
    if options.get('keyword_case'):
        stack.preprocess.append(
            KeywordCaseFilter(options['keyword_case']))

    if options.get('identifier_case'):
        stack.preprocess.append(
            IdentifierCaseFilter(options['identifier_case']))

    if options.get('truncate_strings'):
        stack.preprocess.append(TruncateStringFilter(
            width=options['truncate_strings'], char=options['truncate_char']))

    if options.get('use_space_around_operators', False):
        stack.enable_grouping()
        stack.stmtprocess.append(SpacesAroundOperatorsFilter())
    ...
    if options.get('reindent_aligned', False):
        stack.enable_grouping()
        stack.stmtprocess.append(
            AlignedIndentFilter(char=options['indent_char']))

    if options.get('right_margin'):
        stack.enable_grouping()
        stack.stmtprocess.append(
            RightMarginFilter(width=options['right_margin']))

    # Serializer
    if options.get('output_format'):
        frmt = options['output_format']
        if frmt.lower() == 'php':
            fltr = OutputPHPFilter()
        elif frmt.lower() == 'python':
            fltr = OutputPythonFilter()
        else:
            fltr = None
        if fltr is not None:
